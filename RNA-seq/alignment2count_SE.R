...
if (length(args)<1){
	stop("Usage(single-end RNASeq): Rscript alignment2count_SE.R input fastq directory, output alignment directory, output count directory, index file name, gene annotation\n", call.=F)
...
file_names_out = unique(gsub(".fastq","",file_names))
P1 = paste0(file_dir,"/", file_names)
out_dir1 = paste0(align_dir, "/", file_names_out)

table_cmd = cbind(command = paste("tophat2 --library-type fr-unstranded -p", Ncores, "-G"), Annot=ga, Outfile=paste("-o", out_dir1), BowtieIndex = indexf, P1)

table_cmd2 = cbind(com="samtools sort -n", file=paste0(out_dir1,"/accepted_hits.bam"), out=paste0(out_dir1,"/accepted_hits.sorted"))

table_cmd3 = cbind(com="htseq-count -f bam -r name -s no", file=paste0(out_dir1,"/accepted_hits.sorted.bam"), gtf_dir=ga, out=paste0("> ",count_dir,"/",file_names_out,".count"))

#write.table(table_cmd, "table_tophat.txt", sep=" ", quote=F, row.names=F, col.names=F)

for(i in 1:length(P1)){ 
	out_bsubscript = paste0(file_names_out[i],".txt")
	cat(
paste("#!/bin/bash\n#BSUB -J", file_names_out[i], "\n#BSUB -o %J.align2count.log\n#BSUB -e %J.align2count.err\n#BSUB -W 6:00\n#BSUB -q general\n#BSUB -n 8\n#BSUB -R 'rusage[mem=4096] span[hosts=1]'\n#BSUB -N\n#BSUB -u\n"), file=out_bsubscript)
